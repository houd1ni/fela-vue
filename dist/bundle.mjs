import { when, complement, isNil, replace, both, isEmpty, typeIs, map, ifElse, identity, prop, compose, toPairs, length, last, qmergeDeep, qmap, test, split, fromPairs, qreverse, curry, join as join$1, forEach, type, qfilter, trim, qassoc, all, head, tail, slice, always, eq, mergeShallow, once, propEq, keys, intersection, flat } from 'pepka';
...
function FelaVueCSS(globals = {}) {
    ...
    return {
        ...
        transform(src, id) {
            if (file_re.test(id)) {
                try {
                    const vue = parse$1(src).descriptor;
                    if (vue.template) {
                        const { template } = vue;
...
export { FelaVueCSS, Renderer, SvelteRenderer, __specialcss, css, rollupCSSCompression, setCompression };
