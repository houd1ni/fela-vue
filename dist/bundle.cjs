...
pepka.typeIs('Object');
const isFunction = pepka.typeIs('Function');
const isWindow = pepka.typeIs('Window');
const isBrowser = (() => {
    try {
...
const tryUnwrap = pepka.map(pepka.ifElse(isFunction, pepka.identity, pepka.prop('default')));
...
const parse = (() => {
    ...
    return (css, aug = false) => {
        ...
        }), createFunctions(aug), pepka.qfilter(pepka.complement(pepka.isEmpty)), pepka.qmap(pepka.trim), splitNonEscaped(delimiters), pepka.replace(/(\{|\})/g, (_, brace, offset, full) => {
...
const addClassName = pepka.qassoc('className');
const addName = (name) => pepka.compose(pepka.when(pepka.always(name), addClassName(name)), pepka.when(pepka.eq(emptyObject), pepka.always({})) // Base emptyObject is frozen.
);
    ...
    switch (typeof propsOrRule) {
        ...
        case types.s:
            ...
            for (const [name, rule] of pickStyles(getDefStyle, style, propsOrRule, modifiers, context)) {
                if (classNames && name) {
                    combined_name += (pepka.length(combined_name) ? '-' : '') + name;
...
class Renderer {
    ...
    constructor(opts = {}) {
        ...
        this.renderClasses = (stylesheet, propsOrRule, props = emptyObject, modifiers) => {
            const rules = getRules(pepka.once(() => fdefValue ? fdefValue(this) : emptyObject), stylesheet, propsOrRule, modifiers ? pepka.mergeShallow(opts.modifiers, modifiers) : opts.modifiers, classNames, props);
            return renderer.renderRule(fela.combineRules(...rules), props);
        };
        ...
        // Mixin creation.
        this._mixin = pepka.qfilter(pepka.identity, {
            methods: {
